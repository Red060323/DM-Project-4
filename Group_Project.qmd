...
education_clean <- education_clean %>%
  # Supprimer tout ce qui est après la virgule (y compris la virgule) et les points
  mutate(across(-states, ~ gsub(",.*", "", .))) %>%  # Supprime la virgule et tout après
  mutate(across(-states, ~ gsub("\\.", "", .))) %>%  # Supprime les points
