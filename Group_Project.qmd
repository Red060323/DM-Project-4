...
```
Changing the names of the states by their codes in the education dataset:
```{r}
...
education_clean <- education_clean %>%
  left_join(state_codes, by = c("states" = "state_name")) %>%  # Jointure par le nom des états
  mutate(states = state_code) %>%  # Remplacer les noms par les codes
  select(-state_code)  # Supprimer la colonne temporaire
...
In the following, we are going to proceed to the cleaning of the dataset about lethal overdose and the one about drug seizure that we've already imported. First, in the both datasets, we kept 2022 as this is the only year which we will focus our analysis on, thus we removed all other years.
...
kable(
  head(drug_seizure, 5),
  caption = "Summary of the cleaned drug seizure data set read as a variable: ",
  escape = FALSE
)
```
...
```{r}
#| message: false
lethal_overdoses <- lethal_overdoses %>%
  filter(YEAR == 2022)%>%
  rename(states = STATE)

kable(
  head(lethal_overdoses, 5),
...
# Example: summary of the seizures by drug type
...
fentanyl_data <- drug_seizure %>%
  ...
  group_by(`states`) %>%  # Grouper par région
...
fentanyl_plot <- ggplot(fentanyl_data, aes(
  x = reorder(`states`, Fentanyl_Seizures),  # Réorganiser par nombre de saisies
  y = Fentanyl_Seizures,
  text = paste("State:", `states`, "<br>Seizures of Fentanyl:", Fentanyl_Seizures)
)) +
