...
-   **Discussion**:
    -   For the variable `total_quantity_seized` , the *mean* is **180.63**, indicating a relatively high average quantity of drugs seized in the USA . The *standard deviation* of **312.60** suggests great variability in seizures, indicating that some seizures are much larger than the average. The median, at **4.50**, is significantly lower than the mean, highlighting an asymmetrical distribution, as evidenced by the skew of **5.78**, showing a strong positive asymmetry. This implies that there are several very large seizures pulling the average upwards. Kurtosis, at **41.31**, shows a very sharp distribution with heavy tails, suggesting that extreme values are frequent. 

        Concerning education, the  variable **`years25_atleast_highschool_2022`**, has a mean of 12.86 with a standard deviation of **1.03**, indicating that the majority of individuals have completed at least high school with little variability. The median is **13**, which is close to the mean, and the skew is **0.15**, indicating an almost symmetrical distribution.  In contrast, for**\`years25_bachelor_2022\`**, the mean is **2.12** with a standard deviation of **0.49**, showing fewer people with a bachelor's degree, and a median of 2. The kurtosis of **2.50** indicates a distribution close to normal. For **\`years25_graduate_2022\`**, the mean is **0.37**, and although the standard deviation is **0.49**, this demonstrates that few people have a higher degree, with a skew of **2.14**, highlighting a concentration of low values. This suggests that higher-level diplomas are much less common.

        Finally the variable **`overdose_deaths`**, with a *mean* of **13.80** indicates an average number of fatal overdoses. The standard deviation of **27.69** highlights a high variability in overdose deaths, which is a very significant and alarming. The median is **5.50**, notably lower than the mean, indicating a strong positive  asymetry, as indicated by the skew of **5.29**. This suggests that most values are relatively low, but a few extreme cases of overdose deaths pull the mean upwards. The kurtosis of **36.76** indicates a very sharp distribution with heavy tails, showing that extreme values of overdose deaths are frequent. This may call for targeted interventions to treat extreme cases and better understand the factors underlying these deaths.

So, let's take a closer look at the statistical measure of the median. Here, the variables are classified according to their national median: <br><br> **Education Variables**: classified as "success" if their values are above the national median and "failure" otherwise. <br><br> **Overdose Variables**: classified as "success" if their values are below the national median and "failure" otherwise. <br><br> **Drug Seizure Variables**: classified as "success" if their values are below the national median and "failure" otherwise.
...
### Consumption and Overdoses

```{r, message = FALSE}
joined_dataset <- joined_dataset %>%
  filter(states != "USA")

ggplot(joined_dataset, aes(x = total_quantity_seized, y = overdose_deaths)) +
...
```{r}
# Create the base ggplot with the regression line
interactive_plot <- ggplot(joined_dataset, aes(x = total_quantity_seized, 
                                               y = overdose_deaths, 
...
# Convert the ggplot object into an interactive plotly plot
...
The regression line does not automatically appear so:
...
-   **Comments:**
    ...
    ### Seizure and Education

    Doing the same but with the seizure data set
