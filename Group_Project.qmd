...
So, let's take a closer look at the statistical measure of the median. Here, the variables are classified according to their national median: <br><br> **Education Variables**: classified as "success" if their values are above the national median and "failure" otherwise. <br><br>
**Overdose Variables**: classified as "success" if their values are below the national median and "failure" otherwise. <br><br>
...
<br>
We are now starting to examine the Relationship Between Education and Overdose Rates. (developp)
### Overdose and Education
(we do not keep):
```{r, message = FALSE}
joined_dataset <- joined_dataset %>%
  filter(states != "USA")
...
<br>
The goal here is to check if this negative relationship is statistically significant, using the summary of the linear model.
```{r}
# Fit the linear model
model <- lm(overdose_deaths ~ years25_bachelor_2022, data = joined_dataset)
