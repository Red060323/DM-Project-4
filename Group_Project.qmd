...
# Étape 1 : Charger les données géographiques des États-Unis
us_states <- ne_states(country = "United States of America", returnclass = "sf") %>%
  rename(states = postal)  # Harmoniser les colonnes pour la jointure

# Étape 2 : Jointure des données d'éducation avec les données géographiques
us_education <- us_states %>%
  left_join(education_clean, by = "states")  # Jointure via la colonne harmonisée `states`

# Vérifiez si la jointure a fonctionné
glimpse(us_education)

# Étape 3 : Créer une carte interactive avec mapview
map_education <- mapview(
  us_education,
  zcol = "years25_graduate_2022",  # Colonne utilisée pour la couleur
  layer.name = "Graduate Education Levels (%)",  # Nom de la couche
  col.regions = viridis::viridis(100),  # Palette de couleurs
  legend = TRUE
)

# Appliquer une palette personnalisée
custom_palette <- c("#4daf4a", "#377eb8", "#ff7f00", "#984ea3", "#a65628")  # Palette manuelle
map <- mapview(
  us_education,
  zcol = "years25_graduate_2022",
  layer.name = "Graduate Education Levels (%)",
...
-   **Comments:**
    ...
    Visually, we can see a heterogeneous distribution: some states have darker colors, indicating a greater concentration of higher education graduates, while others, lighter, have fewer. The use of color gradients allows regional variations to be identified at a glance. Interactive pop-ups (not visible on the image, but present on mouse-over) would give more detail on each state, such as its name and the precise value of the percentage of graduates.
...
Right below, I add a new observation called "USA" where each numeric column represents the mean of all corresponding values across the other states, providing a national-level summary for total events and quantities.

```{r}
# Ajouter une ligne USA avec les moyennes des colonnes numériques
drug_seizure_clean <- drug_seizure_clean %>%
  bind_rows(
    drug_seizure_clean %>%
      summarise(
        across(where(is.numeric), mean, na.rm = TRUE),  # Calcul des moyennes pour les colonnes numériques
        states = "USA"
      )
  )
...
# Étape 1 : Charger les données géographiques des États-Unis
us_states <- ne_states(country = "United States of America", returnclass = "sf") %>%
  rename(states = postal)  # Harmoniser la colonne `states`

# Étape 2 : Jointure des données de drug seizures avec les données géographiques
us_drug_seizure <- us_states %>%
  left_join(drug_seizure_clean, by = "states") # Jointure via la colonne `states``
...
map_drug_seizure <- mapview(
  us_drug_seizure,
  zcol = "total_quantity",         # Colonne utilisée pour la couleur
  layer.name = "Drug Seizure Quantity (lbs)",  # Nom de la couche
  col.regions = viridis::viridis(100),  # Palette de couleurs Viridis
  legend = TRUE
)

# Optionnel : Appliquer une palette manuelle (si nécessaire)
custom_palette <- c("#4daf4a", "#377eb8", "#ff7f00", "#984ea3", "#a65628")  # Palette personnalisée
...
map_drug_seizure <- mapview(
  ...
  col.regions = custom_palette,
  legend = TRUE
  )  # Popup affichant l'abréviation et la quantité
```
```{r}
# Étape 4 : Afficher la carte
map_drug_seizure
...
-   **Comments:**
    ...
    This map illustrates the quantity of drugs seized (in pounds) by state in the United States. The data show significant variation between states. California and Arizon record the highest quantities, exceeding 7,000 pounds respectively. On the other hand, several states, notably Alaska, show unavailable data or negligible quantities. The disparities observed may reflect factors such as the importance of drug trafficking routes, the proximity of borders and the priorities of law enforcement in each region.
...
# Étape 3 : Jointure des données sur les overdoses avec les données géographiques
us_overdoses <- us_states %>%
  left_join(lethal_overdoses_clean, by = "states")  # Jointure via la colonne harmonisée `states`

# Vérifiez si la jointure a fonctionné
glimpse(us_overdoses)

# Étape 4 : Créer une carte interactive avec mapview
map_overdose <- mapview(
  us_overdoses,
  zcol = "RATE",                # Colonne utilisée pour la couleur
  layer.name = "Taux d'Overdoses",  # Nom de la couche
  col.regions = viridis::viridis(100),  # Palette de couleurs
  legend = TRUE
)

# Palette manuelle
custom_palette <- c("#4daf4a", "#377eb8", "#ff7f00", "#984ea3")  # 4 couleurs distinctes

# Appliquer la palette personnalisée
map_overdose <- mapview(
  us_overdoses,
  zcol = "RATE",
  layer.name = "Taux d'Overdoses",
  col.regions = custom_palette,
  legend = TRUE
)
```
