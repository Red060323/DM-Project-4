...
header-includes:
  ...
  - \posttitle{\vspace{2cm} \\ \vspace{1cm} \Large \textit{Data Management and Programming} \vspace*{\fill}\end{center}}
filters:
...
To analyze the alignment between education, overdose, and seizure statuses, a composite metric is created for each category (education, overdose, and seizures) by calculating the row-wise mean of relevant variables. States are then classified as "success" or "failure" based on whether their composite score was above or below the national median. Here, the variables are classified:
...
-   $\epsilon 𝑖$: Random error term, assumed to follow a normal distribution N(0,$\sigma 2$)
...
-   [Null hypothesis (H0) :]{.underline}
    ...
    -   There is no statistically significant relationship between the independent variable Xi and the number of overdose deaths. This means that Xi doesn't explain variations in overdose deaths.

-   [Alternative hypothesis (H1):]{.underline}

    -   **H1 :**    $\beta𝑖$ ≠ 0

    -   There is a statistically significant relationship between the independent variable Xi and the number of overdose deaths. In other words, Xi helps explain variations in overdose deaths
...
Our second test, Student test, allows us to measure the difference between the observed mean and $\mu$ adjusted by standard deviation and sample size. Here, we do the 2 Sample t-test, which essentially compares overdose deaths and highschool category. Our hypothesis is:
...
-   **Comments:**
    ...
    Density graph clearly illustrates differences in the distribution of overdose deaths between the "Low" and "Very High" groups. The distribution of deaths for the "Low" group is wider and shows higher mean (4723) than that "Very High" group (1572). However, superposition of distributions indicates high variability in both groups
...
-   **Comments:**
    ...
    The statistics of our regression model show high residual standard error (2187.4975), indicating significant variability in  model's predictions. The multiple R-squared of 0.7298 suggests that 72.98% of the variance in overdose deaths is explained by the included independent variables. However, R-squared adjustment, at 0.4146, reveals that after adjusting for the number of predictors, explanatory power of the model is reduced, thus highlighting potential over-fitting or low robustness. F-statistic analysis (2.32 with 7 and 6 degrees of freedom) isn't significant (p = 0.1629627), indicating that the model doesn't significantly explain variance in deaths. Meaning that although some relationships may exist, the model isn't entirely reliable and require improvements, such as inclusion of additional variables.

## IV. Conclusion

Our research aimed to explore relationship between education levels and fentanyl overdose across U.S. states while examining regional disparities. We provide answers to our initial objectives:

The findings indicate that \uline{states with lower education levels tend to experience higher fentanyl overdose rates}.

First, the analysis of **relationship between education levels and fentanyl overdose rates** reveals weak negative correlation (correlation coefficient of around -0.2). States in the top quartile in terms of education (over 35% of adults with bachelor or higher) have an average overdose rate of 28.5 per 100,000 inhabitants, compared with 36.2 per 100,000 in states in bottom quartile.

Moreover, examination of **regional disparities** shows that Western region has the highest median and variability in total fentanyl seizures, suggesting its potential role as major point of entry or transit for illicit drug trafficking.

After reviewing our research objectives, we'll now assess the extent to which our initial hypotheses have been confirmed by our results.
...
Additionally, the \uline{relationship between fentanyl seizures and overdoses isn't significantly more pronounced in less-educated states}, **contradicting our initial hypothesis**.

Our study has several important limitations. \uline{Firstly}, we didn't take into account confounding factors, notably access to healthcare, that significantly influence overdose rates independently of education.
...
\uline{Finally}, overdose statistics include various drugs, not just fentanyl, which  bias our specific understanding of fentanyl. To address these limitations, we **recommend** incorporating data on access to care in future analyses, extending study period to obtain robust dataset, and seeking data differentiating overdoses by drug type.
